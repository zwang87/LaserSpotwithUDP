...
char buff[MAX_BUF_LEN] = "";
	...
	if(err != 0)
	{
		cout << "socket failed to start up" << endl;
		//return -1;
	}
	...
	while(_running)
	{
		...
		if(_isCalibrated == true){
			...
			circle(image, Point2f(result_x, result_y), 1, Scalar(255, 0, 0), 1);

			sprintf(buff, "pos: %d %d", result_x, result_y);
			int nSendSize = sendto(connect_socket, buff, strlen(buff), 0, (SOCKADDR*)&addr_in, nAddrLen);
			cout << buff << endl;
			//sprintf(buff, "");
